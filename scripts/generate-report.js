...
검색 데이터에 [영상] 표시가 있는 항목(유튜브·방송사 뉴스 영상·웨비나 등 미디어 자료), [연구] 표시가 있는 학술/연구기관 항목, [공공] 표시가 있는 정부·공공기관 항목이 있으면 관련 있는 claim에 srcUrl로 사용하세요 — 기사(웹문서)·영상·학술연구·공공발표를 균형 있게 섞어 쓰세요.
[영상] 표시가 붙은 항목이 검색 데이터에 하나라도 있다면, 이번 JSON 출력 중 최소 1개의 claim은 반드시 그 중 하나를 그대로 srcUrl로 사용하세요 — 이 조건은 선택이 아니라 필수입니다(단, [영상] 항목이 검색 데이터에 전혀 없을 때만 건너뛰세요. 없는 영상 출처를 지어내지 마세요).
